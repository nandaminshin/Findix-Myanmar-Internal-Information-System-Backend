...
func RegisterRoutes(r *gin.Engine, a *App) {
	...
	api := r.Group("/api/fmiis-backend/v001")
	{
		...
		protectedApi.Use(a.AuthMiddleware.Handle())
		{
			...
			protectedApi.POST("/gm-update/:id", a.AuthMiddleware.RequireRole("gm", "md", "hr"), a.UserHandler.GmUpdate)
			protectedApi.POST("/normal-update/:id", a.UserHandler.NormalUpdate)
			...
			protectedApi.GET("/get-all-notifications/:email", a.NotificationHandler.GetAllNotificationsByReceiver)
			protectedApi.GET("/get-single-notification/:id", a.NotificationHandler.GetSingleNotification)
			protectedApi.POST("/mark-notification-as-seen/:id", a.NotificationHandler.MarkNotificationAsSeen)
